        ...
        {filteredRegistrations.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className={`transition-all duration-300 ease-out transform ${isMounted ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} style={{transitionDelay: '200ms'}}><StatCard title={t('dashboard.totalRegs')} value={filteredRegistrations.length} icon={<Icon name="history" className="w-6 h-6" />} /></div>
                <div className={`transition-all duration-300 ease-out transform ${isMounted ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} style={{transitionDelay: '300ms'}}><StatCard title={t('dashboard.commonBreed')} value={mostCommonBreed} icon={<Icon name="ai-sparkles" className="w-6 h-6" />} /></div>
            </div>
            ...
            <div className={`bg-white p-6 rounded-xl shadow-sm border border-cream-200 flex flex-col transition-all duration-500 ease-out transform ${isMounted ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} style={{transitionDelay: '400ms'}}>
              ...
              <div className="flex-grow h-[400px]">
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    <XAxis dataKey="name" angle={-35} textAnchor="end" height={90} tick={{fontSize: 12}} />
